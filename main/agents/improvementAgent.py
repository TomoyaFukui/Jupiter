...
class ImprovementAgent(abstractAgent.AbstractAgent):
    ...
    def make_bid(self, upper, lower):
        ...
        while lower <= self.__utility_space.get_utility_discounted(bid_offer, self.__rule.get_time_now()) and \
    ...
    def sendAction(self):
        ...
        if self.action_index is not None:
            print("step action_index:", self.step_now, self.action_index)
        else:
            print("step action_index:", self.step_now)
        ...
        if self.step_now == 0 and get_step_new(self.max_number_of_steps, self.__rule.get_time_now()) == 0:
            ...
            # upper, lower = self.get_conssetion_values(self.__rule.get_time_now(), 0, 0.1, 1, 1, 0.9, 0.9)
            # if lower < self.__utility_space.get_utility_discounted(self.__opponent_bid, self.__rule.get_time_now()) \
            #     self.__can_accept:
            #     #self.get_conssetion_value() < self.__utility_space.get_utility_discounted(self.__opponent_bid, self.__opponent_action.get_time_offered()) \
            #     return agentAction.Accept(self.__agent_id)
            # else:
        ...
        if self.step_now == get_step_new(self.max_number_of_steps, self.__rule.get_time_now()):
            ...
            # if self.__opponents_action_list[-1] == []:
            #     self.actor.get_action_index
            #     x = [0, 0.1]
            #     y_upper = [1.0, 1.0]
            #     y_lower
            # else:
            ...
            return self.make_action(self.actor.get_action_point_from_index(self.last_action_index),
                                    self.actor.get_action_point_from_index(self.action_index))
            # upper, lower = self.get_conssetion_value(self.__rule.get_time_now(), 0, 0.1, 1, 1, 0.9, 0.9)
            # if lower < self.__utility_space.get_utility_discounted(self.__opponent_bid, self.__rule.get_time_now()) \
            #     self.__can_accept:
            #     #self.get_conssetion_value() < self.__utility_space.get_utility_discounted(self.__opponent_bid, self.__opponent_action.get_time_offered()) \
            #     return agentAction.Accept(self.__agent_id)
...
class QNetwork:
    def __init__(self, learning_rate=0.01, state_size=4, action_size=28, hidden_size=10):
        # 損失関数の定義
    ...
    def replay(self, memory, batch_size, gamma, targetQN):
        ...
        targets = np.zeros((batch_size, 28))
...
class Actor:
    ...
    def get_action_index(self, state, episode, targetQN):   # [C]ｔ＋１での行動を返す
        ...
        if epsilon <= np.random.uniform(0, 1):
            ...
            action = np.random.choice(range(0, self.action_num+1))  # ランダムに行動する
        return action
