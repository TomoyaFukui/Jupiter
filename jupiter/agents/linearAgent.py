...
class LinearAgent(abstractAgent.AbstractAgent):
    ...
    def send_action(self):
        ...
        if self.__opponent_bid is not None and \
            get_conssetion_value() < self.__utility_space.get_utility(self.__opponent_bid):
            return agentAction.Accept(self.__agent_id)

        bid_offer = self.__utility_space.get_bid_above_concession_value(get_conssetion_value())
